---
title: "Geiger-Mueller Dose Calibration – Step-by-Step Procedure"
format: gfm
fig-format: png
fig-dpi: 150
jupyter: python3
...
$$\text{dose}(CPS) = a_2^{(1)} \cdot CPS^2 + a_1^{(1)} \cdot CPS + a_0^{(1)}$$

**If** $CPS > T$:

$$\text{dose}(CPS) = a_2^{(2)} \cdot CPS^2 + a_1^{(2)} \cdot CPS + a_0^{(2)}$$
...
response to flatten or roll off.  The **DOSE\_LIMIT** excludes these
saturated points from the P2 fit:

$$\text{P2 data} = \lbrace (CPS_i, y_i) \mid CPS_i > T \text{ and } y_i < \text{DOSE}\_\text{LIMIT} \rbrace$$
...
The fit uses **weighted least squares** with weights $w_i = 1 / \lvert y_i \rvert$
...
For the points where $CPS > T$ **and** $y < \text{DOSE}\_\text{LIMIT}$ we fit:
...
$$a = m \times 10^{e}, \quad m \in [-32000, 32000], \quad e \in [-9, 9]$$
